...
for dset=1:length(datasets)
    ...
    for binSize_num=1:length(metas_splitHalf)
        ...
        % Haddara2 and Shekhar have an extra dimension (day & contrast)
        if dset<=2
            for meas=1:size(metas_splitHalf{binSize_num},5)
                for bin_num=1:size(metas_splitHalf{binSize_num},2)
                    for day=1:size(metas_splitHalf{binSize_num},3)
                        zSH_binDayorContr(bin_num,day) = r2z(corr(metas_splitHalf{binSize_num}(:,bin_num,day,1,meas), ...
                            metas_splitHalf{binSize_num}(:,bin_num,day,2,meas), 'rows','complete'));
                    end
                end
            ...
            for binSize_num=1:length(metas_splitHalf)
                for meas=1:size(metas_splitHalf{binSize_num},4)
                    for bin_num=1:size(metas_splitHalf{binSize_num},2)
                        zSH_bin(bin_num) = r2z(corr(metas_splitHalf{binSize_num}(:,bin_num,1,meas), ...
                            metas_splitHalf{binSize_num}(:,bin_num,2,meas), 'rows','complete'));
                    end
