"""Regenerate notebook 3 — uses fast (non-MLE) functions only for speed."""
...
"""),
    ...
    md("""## 2. Bootstrap confidence interval
...
"""),
    ...
    md("""## 3. Permutation test — two-condition comparison
...
def permutation_test(stim_a, resp_a, conf_a, stim_b, resp_b, conf_b,
    ...
    for _ in range(n_perm):
        ...
        d = measure_fn(all_stim[perm[:n_a]], all_resp[perm[:n_a]], all_conf[perm[:n_a]]) - \
            measure_fn(all_stim[perm[n_a:]], all_resp[perm[n_a:]], all_conf[perm[n_a:]])
        if not np.isnan(d):
            null.append(d)
...
"""),
    ...
    md("## 4. One-sample t-test across participants"),
    code("""\
...
out = pathlib.Path("docs/tutorials/03_statistical_inference.ipynb")
