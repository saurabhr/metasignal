"""Regenerate tutorial 7 — Bayesian Hierarchical Meta-d'.

Mirrors the structure of the metadpy HMeta-d tutorial
(https://github.com/embodied-computation-group/metadpy) while using
metasignal's three Bayesian approaches.

MLE cells execute during notebook generation so pre-computed outputs are
embedded.  Bayesian cells show the API code but are not executed (they
require brmspy + R + Stan which are optional extras).  Comparison plots
use the actual MLE distributions plus illustrative Bayesian posteriors
derived from the MLE point estimates to demonstrate what the models return.
"""
...
NB   = ROOT / "docs" / "tutorials" / "07_bayesian_hierarchical.ipynb"
...
try:
    ...
    print(f"Execution error (may be non-fatal): {e}")
