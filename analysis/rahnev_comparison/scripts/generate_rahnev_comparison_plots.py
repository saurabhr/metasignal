...
def compute_precision(results: Path, precomp: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    # Prefer paper-protocol Python caches when present (rebuild_protocol_caches.py).
    ha_proto = precomp / "haddara_protocol.npz"
    ma_proto = precomp / "maniscalco_protocol.npz"
    if ha_proto.exists() and ma_proto.exists():
        with np.load(ha_proto, allow_pickle=True) as z:
            py_ha = _precision_dataset(z["precision"], True)
        with np.load(ma_proto, allow_pickle=True) as z:
            py_ma = _precision_dataset(z["precision"], False)
        return mat, np.nanmean([py_ha, py_ma], axis=0)

    # Legacy Haddara-only drops cache (may omit slow MLE columns).
...
def compute_split_half(results: Path, precomp: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    specs = [
        ("Haddara", "results_Haddara.mat", "haddara_protocol.npz", "haddara_mle.npz"),
        ("Shekhar", "results_Shekhar.mat", None, "shekhar_mle.npz"),
        ("Maniscalco", "results_Maniscalco.mat", "maniscalco_protocol.npz", "maniscalco_mle.npz"),
    ]
    for dataset, mf, proto, pf in specs:
        ...
        proto_path = precomp / proto if proto else None
        if proto_path is not None and proto_path.exists():
            with np.load(proto_path, allow_pickle=True) as z:
                py_cell = z["splitHalf"]
            py_values.append(_split_half_from_mat(py_cell, dataset, bin_index=1))
        else:
...
def compute_test_retest(results: Path, precomp: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    proto = precomp / "haddara_protocol.npz"
    if proto.exists():
        with np.load(proto, allow_pickle=True) as z:
            # Cell index 3 = 400-trial bins (matches MATLAB ana_testRetest flip).
            py_arr = np.asarray(z["testRetest"].flat[3], float)
        if py_arr.ndim == 3:
            py_arr = py_arr[:, None, :, :]
        py4 = _remove_test_retest_outliers(py_arr)
    else:
