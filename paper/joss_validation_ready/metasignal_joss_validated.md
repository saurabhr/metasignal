...
Many metacognitive measures have been proposed, but they quantify different properties and respond differently to task performance, response bias, confidence bias, and sample size. Meta-d' is widely used because it expresses metacognitive sensitivity in the same SDT units as perceptual sensitivity [@maniscalco2012]. Its conventional implementation, however, was distributed primarily as MATLAB code [@maniscalco2014]. Other measures were spread across papers and software projects. This made it difficult to compare measures within one reproducible Python workflow.
...
The MATLAB meta-d' code of @maniscalco2014 remains an important reference implementation. `metadpy` [@legrand2021] and HMeta-d [@fleming2017] provide Bayesian or hierarchical estimation, but focus mainly on the meta-d' family. `statConfR` provides models of decision confidence and information-theoretic measures in R [@rausch2025]. These tools are valuable for their intended tasks, but none offers the complete @rahnev2025 benchmark through one pure-Python interface.
...
Ten subject-level analysis arrays were compared: raw estimates, metacognitive-bias recoding, odd--even splits, difficulty conditions, and response-bias conditions. For each measure we examined Pearson correlation, mean absolute error, root-mean-square error, signed bias, maximum absolute error, and missing-value agreement. We also reproduced 19 statistical checks from the supplementary analyses and compared the 17-measure profiles for task-performance dependence, metacognitive-bias dependence, response-bias dependence, split-half reliability, and test--retest reliability. Reliability and precision profiles were computed with the same bin sizes (50, 100, 200, 400 trials), day structure, sequential confidence-alteration rule, and Fisher-*z* aggregation used by the MATLAB scripts, so that these protocol-sensitive analyses are compared like-for-like.
...
The analysis profiles were essentially identical across sources: task-performance dependence (paper--Python *r* = 1.000; MATLAB--Python *r* = 1.000), metacognitive-bias dependence (paper--Python *r* = 1.000; MATLAB--Python *r* = 1.000), and response-bias dependence (paper--Python *r* = 1.000; MATLAB--Python *r* = 1.000). After matching the MATLAB reliability protocol (identical bin sizes, day structure, and Fisher-*z* aggregation), split-half reliability (MATLAB--Python *r* = 0.992), test--retest ICC (MATLAB--Python *r* = 0.999), and precision (MATLAB--Python *r* = 0.996) also aligned closely.
...
No single measure is optimal for every design [@rahnev2025]. Model-based measures can be unstable with small samples or sparse confidence categories. Ratio measures can become extreme when their expected denominator approaches zero. Reliability estimates depend on bin size and resampling design. The optional Bayesian and information-theoretic components have broader dependencies and should not be interpreted as direct replacements for the 17-measure benchmark. Users should choose measures based on their scientific question and report analysis settings completely.
