...
While `metasignal` perfectly reproduces the core task-performance, metacognitive-bias, and response-bias benchmark profiles from @rahnev2025 ($r = 1.000$), split-half reliability and precision validation require nuanced interpretation. An earlier version of this analysis misread @rahnev2025's bin-size convention, treating a "100-trial bin" as a 100-trial window split into 50/50 halves rather than two 100-trial halves (a 200-trial window), per the paper's explicit definition. Correcting this and applying the published split-half protocol (100-trial-per-half bins, Fisher-$z$ averaged across non-overlapping bin instances) directly to raw trial data reproduces the relative ranking of split-half reliability across all 17 measures ($r = 0.983$); it correctly identifies `ΔConf` as highest and the SDT-normalized Ratio measures (Phi-Ratio lowest) as among the least reliable, consistent with the published ranking. It still underestimates absolute reliability values, however (e.g., meta-noise yields 0.24 here versus 0.84 published).

Aggregating the MATLAB split-half arrays bundled with this repository through the same procedure (matching the bundled `ana_splitHalf.m`'s own Fisher-$z$ averaging, with no Python computation involved) yields the same systematic reduction (e.g., meta-$d'$: 0.52 versus 0.89 published). That rules out a `stdpy` computation error. The bundled MATLAB generation code contains no repeated or resampled odd/even splits beyond the fixed non-overlapping bins already implemented here, so the residual gap reflects a difference between the shipped reference materials and whatever procedure originally produced Figure 7, rather than a recoverable detail in the Methods prose. Under a bin-instance cap required for computational tractability, precision estimates display similar shortfalls and elevated sampling noise, occasionally causing unstable measures (e.g., Gamma-Ratio) to invert signs. Consequently, current split-half and precision figures should be treated as illustrative rather than exact protocol matches to @rahnev2025.
