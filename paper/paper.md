...
![Computational architecture of `metasignal`. Trial-level arrays flow top-down through four layers: (1) `stdpy` computes SDT statistics and all twenty metacognitive measures; (2) `compute_all_measures` returns the full 26-element output; (3) `analysis` provides bootstrap CIs, permutation tests, and group summaries, and the CLI exposes the full measure suite without Python code; (4) two experimental, pre-1.0 components — `sdtbayes`, offering three Bayesian estimation approaches that return a `FitResult` with shared diagnostics, and `itmc`, an information-theoretic metacognition module.](structure.png)
